#!/usr/bin/python3
"""
Title: 9-rectangle.py
Description: Write a class Rectangle that defines a rectangle by:
            (based on 8-rectangle.py)
Author: @a_idk Scripting
"""


class Rectangle:
    """
    Class Name: Rectangle
    Class Attributes:
        number_of_instances: reps the no. of instances of shape
        print_symbol: string symbol used
    """
    print_symbol = "#"
    number_of_instances = 0

    def __init__(self, width=0, height=0):
        """
        Initializing the class instance 'Rectangle'
        Args:
            height: instance for height of Rectangle
            width: instance for width of Rectangle
        """
        self.width = width
        self.height = height
        type(self).number_of_instances += 1

    @property
    def width(self):
        """ width getter """
        return self.__width

    @width.setter
    def width(self, value):
        """
        width setter
        Args:
            value: positive integer width value
        """
        if not isinstance(value, int):
            raise TypeError("width must be an integer")
        if value < 0:
            raise ValueError("width must be >= 0")
        self.__width = value

    @property
    def height(self):
        """ height getter """
        return self.__height

    @height.setter
    def height(self, value):
        """
        height setter
        Args:
            value: positive integer height value
        """
        if not isinstance(value, int):
            raise TypeError("height must be an integer")
        if value < 0:
            raise ValueError("height must be >= 0")
        self.__height = value

    def perimeter(self):
        """
        return perimeter of rectangle
        """
        if self.__width == 0:
            return (0)
        elif self.__height == 0:
            return (0)
        return ((self.__width * 2) + (2 * self.__height))

    def area(self):
        """
        Return the area of the rectangle
        """
        return (self.__height * self.__width)

    def __str__(self):
        """
        Return the printable representation of the Rectangle.
        """
        if self.__width == 0:
            return ("")
        elif self.__height == 0:
            return ("")

        # Using '#' to create the rectangle
        shape = []  # initializing the shape
        for x in range(self.__height):
            [shape.append(str(self.print_symbol)) for y in range(self.__width)]
            if x != self.__height - 1:
                shape.append("\n")
        return ("".join(shape))

    def __repr__(self):
        """
        Return official string representation of shape
        """
        shape = "Rectangle(" + str(self.__width)
        shape = shape + ", " + str(self.__height) + ")"
        return shape

    @staticmethod
    def bigger_or_equal(rect_1, rect_2):
        """
        Comparing Shapes
        Return:
            the biggest shape
        Args:
            rect_1: first shape(rectangle)
            rect_2: second shape(rectangle)
        Raises:
            TypeError: If either rect_1 / rect_2 is not rectangle
        """
        if not isinstance(rect_1, Rectangle):
            raise TypeError("rect_1 must be an instance of Rectangle")
        if not isinstance(rect_2, Rectangle):
            raise TypeError("rect_2 must be an instance of Rectangle")
        if rect_1.area() >= rect_2.area():
            return rect_1
        return rect_2

    @classmethod
    def square(cls, size=0):
        """
        Returns new Rectangle with width == height == size

        Args:
            cls (class): the class itself
            size (int): size of the new Rectangle
        """
        return cls(size, size)

    def __del__(self):
        """
        Destructor method with message 'Bye rectangle...'
        """
        type(self).number_of_instances -= 1
        print("Bye rectangle...")
